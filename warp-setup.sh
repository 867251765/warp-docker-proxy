...
if [ -s "/var/lib/cloudflare-warp/reg.json" ]; then
  ...
  sudo -u cloudflare warp-cli --accept-tos mode warp

  # registry warp (free/zero trust)
  if [ -z "$WARP_TOKEN" ]; then
      # 注册 free 账号
      sudo -u cloudflare warp-cli --accept-tos registration new
      if [ -n "$WARP_LICENSE" ]; then
        echo "set WARP License..."
        sudo -u cloudflare warp-cli registration license "$WARP_LICENSE"
      fi
  else
      # 注册 zero trust 账户
      TEAM_NAME=$(echo "$WARP_TOKEN" | grep -oP '(?<=//)[^/]+')
      TEAM_NAME=${TEAM_NAME%%.*}
      sudo -u cloudflare warp-cli --accept-tos registration new "$TEAM_NAME"
      sudo -u cloudflare warp-cli --accept-tos registration initialize-token-callback
      sudo -u cloudflare warp-cli --accept-tos registration token "$WARP_TOKEN"
  fi

  # 检查注册状态
  echo "registration status"
  sudo -u cloudflare warp-cli --accept-tos registration show
  # 连接warp
  echo "connect warp"
  sudo -u cloudflare warp-cli --accept-tos connect

  # Loop to check if WARP is Connected
  RETRY_COUNT=1
  while true; do
      status=$(sudo -u cloudflare warp-cli --accept-tos status)
...
while true; do
    ...
    # 捕获状态，即使命令失败也不让脚本退出
    status=$(sudo -u cloudflare warp-cli --accept-tos status 2>/dev/null || echo "disconnected")

    if [[ $status != *"Connected"* ]]; then
        echo "WARP connection lost. Attempting to reconnect... (Attempt $((RETRY_COUNT+1)))"

        # 尝试重连，如果重连失败，等待时间逐渐增加 (5s, 10s, 15s...)
        if ! sudo -u cloudflare warp-cli --accept-tos connect; then
